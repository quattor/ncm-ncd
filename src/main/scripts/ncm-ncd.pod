...
(See C<ncm-ncd --help> for full list of options with default values.)
...
In case of deinstallation of a component using C<--unconfigure>,
...
It is enabled by default, use C<--no-check-noquattor> to disable it
(or set C<check-noquattor = 0> in the configfile).
...
location of the configuration file is defined in the C<--cfgfile>
...
it was called with the C<--noaction> flag.
